...
{
    class RVEExamplesBuilder_Debug_ody
    {       
        ...
        public static void OriginalRVECholExample(Model model)
        {
            ...
            rveMatrixParameters mp;
            grapheneSheetParameters gp;
            mpgp =RVEExamplesBuilder_Debug_ody.GetOriginalRveCholExampleParameters();
            mp = mpgp.Item1;
            gp = mpgp.Item2;
            double[] ekk_xyz = new double[] { 0, 0, 0 };
            RVEExamplesBuilder_Debug_ody.HexaElementsOnlyRVE(model, mp, Dq);
            int hexaElementsNumber = model.ElementsDictionary.Count();
            RVEExamplesBuilder_Debug_ody.AddGrapheneSheet2(model, gp,ekk_xyz);
            int shellElementsNumber = (model.ElementsDictionary.Count() - hexaElementsNumber) / 3;
            // model: add loads NA ALLAXTHEI TO PATH
            //RVEExamplesBuilder_Debug_ody.AddLoadsOnRveFromFile(model,mp.hexa1,mp.hexa2,mp.hexa3, @"C:\Users\turbo-x\Desktop\cohesive_check_MSOLVE_2\paradeigma_apo_arxika_swsta_embeded_shell_gia_check_tou_rve_embedding_sto_MSolve\fe2_tax_me1_arxiko_chol_dixws_me1_OriginalRVEExampleChol\Fxk_p_komvoi_rve.txt");
            RVEExamplesBuilder_Debug_ody.AddLoadsOnRveFromFile(model, mp.hexa1, mp.hexa2, mp.hexa3, @"C:\Users\turbo-x\Desktop\cohesive_check_MSOLVE_2\paradeigma_apo_arxika_swsta_embeded_shell_gia_check_tou_rve_embedding_sto_MSolve\fe2_tax_me1_arxiko_chol_dixws_me1_OriginalRVEExampleChol_me_a1\Fxk_p_komvoi_rve.txt");
            // model: add constraints
            RVEExamplesBuilder_Debug_ody.AddConstraintsForNonSingularStiffnessMatrix(model, mp.hexa1, mp.hexa2, mp.hexa3);
        ...
        public static void OneElementRVECheckExampleConstrained(Model model)
        {
            ...
            rveMatrixParameters mp;
            grapheneSheetParameters gp;
            mpgp = RVEExamplesBuilder_Debug_ody.GetOneElementRveCheckExampleParameters();
            mp = mpgp.Item1;
            gp = mpgp.Item2;
            double[] ekk_xyz = new double[] { 0, 0, 0 };
            RVEExamplesBuilder_Debug_ody.HexaElementsOnlyRVE(model, mp,Dq);
            int hexaElementsNumber = model.ElementsDictionary.Count();
            RVEExamplesBuilder_Debug_ody.AddGrapheneSheet(model, gp,ekk_xyz); // kai edw thelei graphene sheet2 an kai den paizei rolo 
            int shellElementsNumber = (model.ElementsDictionary.Count()-hexaElementsNumber)/3;
            // model: add loads
            //RVEExamplesBuilder_Debug_ody.AddLoadsOnRveFromFile(model, mp.hexa1, mp.hexa2, mp.hexa3, @"C:\Users\turbo-x\Desktop\cohesive_check_MSOLVE_2\paradeigma_apo_arxika_swsta_embeded_shell_gia_check_tou_rve_embedding_sto_MSolve\elegxos_alalgwn_fe2_tax_me1_arxiko_chol_dixws_me1_OneElementRVECheckExample\Fxk_p_komvoi_rve.txt");
            RVEExamplesBuilder_Debug_ody.AddXLoadsOnYZConstrainedOneElementRVE(model);
            // model: add constraints
            //RVEExamplesBuilder_Debug_ody.AddConstraintsForNonSingularStiffnessMatrix(model, mp.hexa1, mp.hexa2, mp.hexa2);
            RVEExamplesBuilder_Debug_ody.AddConstraintsForYZConstraindeOneElementRVE(model);
            var embeddedGrouping = new EmbeddedCohesiveGrouping(model, model.ElementsDictionary.Where(x => (x.Key < hexaElementsNumber+1) ).Select(kv => kv.Value), model.ElementsDictionary.Where(x => (x.Key>=shellElementsNumber+hexaElementsNumber+1)).Select(kv => kv.Value));
        ...
        public static void FewElementsRVECheckExample2GrapheneSheets(Model model)
        {
            ...
            rveMatrixParameters mp;
            grapheneSheetParameters gp;
            mpgp = RVEExamplesBuilder_Debug_ody.Get2GrapheneSheetsRveExampleParameters();
            mp = mpgp.Item1;
            gp = mpgp.Item2;
            ...
            {
                RVEExamplesBuilder_Debug_ody.AddGrapheneSheet_with_o_x_parameters(model, gp, ekk_xyz[j], model_o_x_parameteroi[j]);
        ...
        public static void Reference1RVEExample10000(Model model)
        {
            ...
            int discr1_shell = 1;
            mpgp = RVEExamplesBuilder_Debug_ody.GetReferenceRveExampleParameters(subdiscr1, discr1, discr3, subdiscr1_shell, discr1_shell);
            mp = mpgp.Item1;
            gp = mpgp.Item2;
        ...
        public static void Reference2RVEExample50000withRenumbering(Model model)
        {
            ...
            RVEExamplesBuilder_Debug_ody.HexaElementsOnlyRVEwithRenumbering(model, mp, Dq, renumbering_vector_path);
            int hexaElementsNumber = model.ElementsDictionary.Count();
            ...
            for (int j = 0; j < graphene_sheets_number; j++)
            {
                RVEExamplesBuilder_Debug_ody.AddGrapheneSheet_with_o_x_parameters_withRenumbering(model, gp, ekk_xyz[j], model_o_x_parameteroi[j],renumbering_vector_path);
                shellElementsNumber = (model.ElementsDictionary.Count() - element_counter_after_Adding_sheet) / 3; //tha xrhsimefsei
                //embdeddedGroup_adittion= model.ElementsDictionary.Where(x => (x.Key >= shellElementsNumber + element_counter_after_Adding_sheet + 1)).Select(kv => kv.Value);
                //embdeddedGroup.Concat(embdeddedGroup_adittion);
                for (int k = shellElementsNumber + element_counter_after_Adding_sheet + 1; k < model.ElementsDictionary.Count() + 1; k++)
            ...
            // model: add loads
            RVEExamplesBuilder_Debug_ody.AddLoadsOnRveFromFile_withRenumbering(model, mp.hexa1, mp.hexa2, mp.hexa3, Fxk_p_komvoi_rve_path, renumbering_vector_path);
            //RVEExamplesBuilder_Debug_ody.AddXLoadsOnYZConstrainedOneElementRVE(model);
            // model: add constraints
            RVEExamplesBuilder_Debug_ody.AddConstraintsForNonSingularStiffnessMatrix_withRenumbering(model, mp.hexa1, mp.hexa2, mp.hexa3, renumbering_vector_path);
            //RVEExamplesBuilder_Debug_ody.AddConstraintsForYZConstraindeOneElementRVE(model);
        ...
        public static void Reference2RVEExample10000withRenumberingwithInput(Model model)
        {
            ...
            for (int j = 0; j < graphene_sheets_number; j++)
            {
                ...
                RVEExamplesBuilder_Debug_ody.AddGrapheneSheet_with_o_x_Input_withRenumbering(model, gp, ekk_xyz[j], model_o_x_parameteroi[j], renumbering_vector_path, ox_sunol_input_path);
                shellElementsNumber = (model.ElementsDictionary.Count() - element_counter_after_Adding_sheet) / 3; //tha xrhsimefsei
                //embdeddedGroup_adittion= model.ElementsDictionary.Where(x => (x.Key >= shellElementsNumber + element_counter_after_Adding_sheet + 1)).Select(kv => kv.Value);
                //embdeddedGroup.Concat(embdeddedGroup_adittion);
                for (int k = shellElementsNumber + element_counter_after_Adding_sheet + 1; k < model.ElementsDictionary.Count() + 1; k++)
