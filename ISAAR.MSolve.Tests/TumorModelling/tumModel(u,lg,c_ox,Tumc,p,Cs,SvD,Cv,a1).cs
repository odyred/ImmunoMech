...
{
	public class tumModel_u_lg_c_ox_Tumc_p_Cs_SvD_Cv_a1_
	{
		...
		private static readonly double[] Aox = new double[] { 2200.0d / 24d / 3600d, 2200.0d / 24d / 3600d }; //mol/(m^3*s)
		...
		private static Tuple<Model, IModelReader> oxModel, gModel, ctModel, prModel, csModel, SvDModel, cvModel, a1Model, structModel;
		...
		private static void RunTest()
		{
			...
			var models = new[] { SvDModel.Item1, oxModel.Item1, ctModel.Item1, gModel.Item1, 
				prModel.Item1, csModel.Item1, csModel.Item1, a1Model.Item1 };
			var modelReaders = new[] { SvDModel.Item2, oxModel.Item2, ctModel.Item2, gModel.Item2, 
				prModel.Item2, csModel.Item2, cvModel.Item2, a1Model.Item2 };
		...
		private static void Paraview(int timeStep)
		{
			...
			using (StreamWriter outputFile = new StreamWriter(path3))
			{
				...
				for (int i = SvDNode.Length; i < numberOfPoints; i++)
					outputFile.WriteLine($"0 ");
				outputFile.WriteLine("              </DataArray>");
				...
				for (int i = CvNode.Length; i < numberOfPoints; i++)
					outputFile.WriteLine($"0 ");
				outputFile.WriteLine("              </DataArray>");
		...
		private static void OxygenTransportCoefficientsCalculation(Dictionary<int, double[]> u, Dictionary<int, double> l)
		{
			...
			foreach (var e in SvDModel.Item2.elementDomains[0])
			{
				...
				u[e.ID][0] = 24 * 3600 * (-khy[0] * dpElement[e.ID][0] / .7 + vElement[e.ID][0]);
				u[e.ID][1] = 24 * 3600 * (-khy[0] * dpElement[e.ID][1] / .7 + vElement[e.ID][1]);
				u[e.ID][2] = 24 * 3600 * (-khy[0] * dpElement[e.ID][2] / .7 + vElement[e.ID][2]);
				duof[e.ID] = -khy[0] * ddpElement[e.ID][0] / .7 + uXt[e.ID][0];
				dvof[e.ID] = -khy[0] * ddpElement[e.ID][1] / .7 + uXt[e.ID][1];
				dwof[e.ID] = -khy[0] * ddpElement[e.ID][2] / .7 + uXt[e.ID][2];
				l[e.ID] = 24 * 3600 * (Dox[0] / Lwv * Svin * dd0[e.ID] * SvDElement[e.ID]
			...
			foreach (var e in SvDModel.Item2.elementDomains[1])
			{
				...
				u[e.ID][0] = 24 * 3600 * (-khy[1] * dpElement[e.ID][0] / .7 + vElement[e.ID][0]);
				u[e.ID][1] = 24 * 3600 * (-khy[1] * dpElement[e.ID][1] / .7 + vElement[e.ID][1]);
				u[e.ID][2] = 24 * 3600 * (-khy[1] * dpElement[e.ID][2] / .7 + vElement[e.ID][2]);
				duof[e.ID] = -khy[1] * ddpElement[e.ID][0] / .7 + uXt[e.ID][0];
				dvof[e.ID] = -khy[1] * ddpElement[e.ID][1] / .7 + uXt[e.ID][1];
				dwof[e.ID] = -khy[1] * ddpElement[e.ID][2] / .7 + uXt[e.ID][2];
				l[e.ID] = 24 * 3600 * (Dox[1] / Lwv * Svin * SvDElement[e.ID]
		...
		private static void SvDCoefficientsCalculation(IList<IList<int>> elementDomains, Dictionary<int, double> k, Dictionary<int, double[]> u, Dictionary<int, double> l)
		{
			if (SvDElement == null)
			{
				SvDElement = new double[u.Count];
				foreach (var id in elementDomains[0])
				{
					SvDElement[id] = 0.5;
				}
				foreach (var id in elementDomains[1])
				{
					SvDElement[id] = 1;
				}
			}
			...
			for (int i = 0; i < u.Count; i++)
			{
				k[i] = 24 * 3600 * Dsv * Math.Pow((1 + s1 * a1Element[i] * a10), -aD) * Math.Pow((1 + s2 * a20), bD);
			}
			SvDK = k;
			if (dCsElement == null || dCvElement == null)
			{
				...
				for (int i = 0; i < u.Count; i++)
				{
					u[i] = new double[] { 24 * 3600 * xn * (dCvElement[i][0] * Cv0 + WvsSv * dCsElement[i][0] * Cs0),
						24 * 3600 * xn * (dCvElement[i][1] * Cv0 + WvsSv * dCsElement[i][1] * Cs0),
						24 * 3600 * xn * (dCvElement[i][2] * Cv0 + WvsSv * dCsElement[i][2] * Cs0)};
				}
			SvDU = u;
			if (ddCsElement == null || ddCvElement == null)
			{
				...
				{
					l[i] = 24 * 3600 * xn * (Cv0 * ddCvElement[i].Sum() + WvsSv * Cs0 * ddCsElement[i].Sum());
				}
			SvDL = l;
		}
		private static void CvCoefficientsCalculation(IList<IList<int>> elementDomains, Dictionary<int, double> k, Dictionary<int, double[]> u, Dictionary<int, double> l)
		{
			foreach (IList<int> domain in elementDomains)
			{
				foreach (var id in domain)
				{
					k[id] = Dvegf[elementDomains.IndexOf(domain)] * 24 * 3600;
				}
			}
			CvK = k;
			CvU = u;
			foreach (var id in elementDomains[0])
			{
				l[id] = 24 * 3600 * (l11 * Svin * SvDElement[id] + l13);
			}
			CvL = l;
		}
			...
			IConvectionDiffusionIntegrationProvider[] providersToReplace, IChildAnalyzer[] childAnalyzersToReplace)
		{
			...
			var c_oxFirstDerivatives = providersToReplace[1].GetFirstSpaceXDerivatives(c_oxSubdomain, Vector.CreateFromArray(c_oxNode));
			...
			var CsFirstDerivatives = providersToReplace[5].GetFirstSpaceXDerivatives(CsSubdomain, Vector.CreateFromArray(CsNode));
			...
			var CvFirstDerivatives = providersToReplace[6].GetFirstSpaceXDerivatives(CvSubdomain, Vector.CreateFromArray(CvNode));
			...
			foreach (int domainID in domainIDs)
			{
				foreach (Element element in modelReader.elementDomains[domainID])
				{
					...
					if (domainID == 0)
						...
						RTumc = 0;
					var fp = RTumc + pv * PressureL[element.ID] - uXt[element.ID].Sum() * 24 * 3600;
					var nodes = (IReadOnlyList<Node>)element.Nodes;
		...
		private static Tuple<Model, IModelReader> CreateAng1Model()
		{
			...
			if (a1Element == null) lgElement = new double[oxModel.Item1.Elements.Count];
