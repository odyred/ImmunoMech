...
{
    public class HyperElasticGrowthOxyTumPressure
    {
        ...
        private static Tuple<Model, IModelReader> oxModel, gModel, ctModel, prModel, structModel;
        ...
        private static void RunTest()
        {
            ...
            var models = new[] { oxModel.Item1, ctModel.Item1, gModel.Item1, prModel.Item1 };
            var modelReaders = new[] { oxModel.Item2, ctModel.Item2, gModel.Item2, prModel.Item2 };
        ...
        private static Tuple<Model, IModelReader> CreatePressureModel(double[] k)
        {
            ...
            if (prModel == null)
            {
                ...
                modelReader = new ComsolMeshReader4(filename, k, PressureCoefficientsCalculation);
                model = modelReader.CreateModelFromFile(modelDomains, modelBoundaries);
            ...
            else
            {
                ...
                modelReader = modelReader.UpdateModelReader(k, PressureU, PressureL);
                model = modelReader.UpdateModel(structModel.Item1, Displacements);
            }
            ...
            foreach (int domainID in domainIDs)
            {
                foreach (Element element in modelReader.elementDomains[domainID])
                {
                    var prMaterial = new ConvectionDiffusionMaterial(k[domainID], conv0[0], PressureL[element.ID]);
                    Grox[element.ID] = (loxc[domainID] * cvox * c_oxElement[element.ID]) / (cvox * c_oxElement[element.ID] + Koxc[domainID]);
                    RTumc[element.ID] = 24d * 3600d * Grox[element.ID];
