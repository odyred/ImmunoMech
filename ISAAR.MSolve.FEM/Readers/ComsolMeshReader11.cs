...
{
    public class ComsolMeshReader11 : IModelReader
    {
        ...
        public ComsolMeshReader11(string filename, double[] youngModulus, double[] poissonRatio, IDynamicMaterial[] commonDynamicProperties)
        {
            Filename = filename;
        ...
        public ComsolMeshReader11(string filename, double[] youngModulus, double[] poissonRatio, IDynamicMaterial[] commonDynamicProperties, double[] lambdag)
        {
            Filename = filename;
            this.YoungModulus = youngModulus;
            this.PoissonRatio = poissonRatio;
                ...
                switch (name)
                {
                    ...
                    case Attributes.tet:
                        ...
                        for (int TetID = 0; TetID < NumberOfTetElements; TetID++)
                        {
                            ...
                            ElasticMaterial3DDefGrad elasticMaterial = new ElasticMaterial3DDefGrad() 
                                { 
                                    YoungModulus = YoungModulus[elementDomainID-1], PoissonRatio = PoissonRatio[elementDomainID - 1] 
                                };
                            ContinuumElement3DNonLinearDefGradFactory elementFactory;
                            if (lambdag == null) 
                            {
                                elementFactory = new ContinuumElement3DNonLinearDefGradFactory(elasticMaterial,
                    ...
                    case Attributes.hex:
                        ...
                        for (int HexID = 0; HexID < NumberOfHexElements; HexID++)
                        {
                            ...
                            ElasticMaterial3DDefGrad elasticMaterial = new ElasticMaterial3DDefGrad()
                            {
                                YoungModulus = YoungModulus[elementDomainID - 1],
                                PoissonRatio = PoissonRatio[elementDomainID - 1],
                            };
                            ContinuumElement3DNonLinearDefGradFactory elementFactory;
                            if (lambdag == null)
                            {
