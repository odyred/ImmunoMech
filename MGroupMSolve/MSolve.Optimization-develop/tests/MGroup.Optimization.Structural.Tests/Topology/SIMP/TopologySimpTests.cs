...
using MGroup.Optimization.Algorithms.GradientBased.OC;
...
using MGroup.Optimization.Structural.Topology.SIMP;
using MGroup.Optimization.Structural.Topology.SIMP.Analysis;
using MGroup.Optimization.Structural.Topology.SIMP.Filtering;
using MGroup.Optimization.Structural.Topology.SIMP.MaterialInterpolation;
using MGroup.Solvers.Direct;
using Xunit;

namespace MGroup.Optimization.Structural.Tests.Topology.SIMP
{
	/// <summary>
	...
	public static class TopologySimpTests
	{
		...
		private static void TestCantileverBeam(ILinearFemAnalysis fem, IDensityFilter filter)
		{
			...
			string densitiesPath = Directory.GetParent(Directory.GetCurrentDirectory()).Parent.Parent.FullName
				+ @"\MGroup.Optimization.Structural.Tests\Resources\topology99lines_shortcantilever_densities.txt";
		...
		private static void TestCantileverBeamWith2LoadCasesHardCoded()
		{
			...
			// Check the optimum element densities (design variables).
			string densitiesPath = Directory.GetParent(Directory.GetCurrentDirectory()).Parent.Parent.FullName
				+ @"\MGroup.Optimization.Structural.Tests\Resources\topology99lines_cantilever_loadcases_densities.txt";
			var reader = new FullMatrixReader(false, ',');
			Vector densitiesExpected = reader.ReadFile(densitiesPath).Reshape(false);
		...
		private static void TestMbbBeam(ILinearFemAnalysis fem, IDensityFilter filter)
		{
			...
			// Check the optimum element densities (design variables).
			string densitiesPath = Directory.GetParent(Directory.GetCurrentDirectory()).Parent.Parent.FullName
				+ @"\MGroup.Optimization.Structural.Tests\Resources\topology99lines_MBBbeam_densities.txt";
			var reader = new FullMatrixReader(false, ',');
			Vector densitiesExpected = reader.ReadFile(densitiesPath).Reshape(false);
